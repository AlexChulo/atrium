...
fun <T> subjectLessTestSetup(
    ...
    describe("$groupPrefix -- check expectation functions can be used in an ${AssertionGroup::class.simpleName} with an ${ExplanatoryAssertionGroupType::class.simpleName} and does not throw") {
...
fun <T> subjectLessTestFactory(
    groupPrefix: String,
    vararg assertionCreator: Pair<String, Expect<T>.() -> Unit>
): Any = testFactory(subjectLessTestSetup(groupPrefix, *assertionCreator))
